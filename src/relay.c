...
usage(char const *argv0)
{
    ...
    fprintf(stderr, "   -X, --fastpath -- Optional TC/eBPF session forwarding (Linux 6.6+)\n");
    fprintf(stderr, "   --fastpath-debug -- Enable fastpath and session/statistics logs\n");
...
main(int argc, char *argv[])
{
    ...
    static const struct option longOptions[] = {
        {"fastpath", no_argument, NULL, 'X'},
        {"fastpath-debug", no_argument, NULL, 256},
        {NULL, 0, NULL, 0}
    };
    ...
    while((opt = getopt_long(argc, argv, "hC:S:B:n:i:FX", longOptions, NULL)) != -1) {
	switch(opt) {
	case 256:
	    fastpathDebug = 1;
	    /* fall through */
    ...
    /* Initialize after daemonization so its descriptor cleanup cannot close
       BPF links. Signal handlers only wake the main loop, never call libbpf. */
    if (FastpathRequested) {
	int flags = fcntl(CleanPipe[1], F_GETFL);
	if (flags < 0 || fcntl(CleanPipe[1], F_SETFL, flags | O_NONBLOCK) < 0)
	    fatalSys("fcntl(clean pipe)");
	sa.sa_handler = stopHandler;
	sa.sa_flags = 0;
	if (sigaction(SIGTERM, &sa, NULL) < 0 || sigaction(SIGINT, &sa, NULL) < 0)
	    fatalSys("sigaction(stop)");
	atexit(fastpath_cleanup);
	fastpath_init(Interfaces, NumInterfaces, MaxSessions, fastpathDebug, !beDaemon);
    }
    ...
    while (ActiveSessions) freeSession(ActiveSessions, "Relay shutdown");
    fastpath_cleanup();
    return EXIT_SUCCESS;
}
...
relayLoop()
{
	...
	int eventFD = fastpath_event_fd();
	int selectMax = maxFD;
	readableCopy = readable;
	if (eventFD >= 0) {
	    FD_SET(eventFD, &readableCopy);
	    if (eventFD >= selectMax) selectMax = eventFD + 1;
	}
	r = select(selectMax, &readableCopy, NULL, NULL, fastpath_enabled() ? &wait : NULL);
	if (StopRequested) break;
	if (r < 0 && errno == EINTR) {
	    /* SIGALRM can interrupt every select before its one-second timeout. */
	    fastpath_tick(ActiveSessions, Epoch);
	    continue;
	}
	if (r < 0) {
	...
	/* Refresh kernel activity before the original idle cleaner can run. */
	fastpath_tick(ActiveSessions, Epoch);
